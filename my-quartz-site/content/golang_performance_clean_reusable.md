...
### track var ( type ) refrence - usage
وقتی به یه فانکشن آدرس یه وریبل رو میدیم ، به راحتی می تونیم درون تابع مقدارشو تغییر بدیم

اما توی تریس وقتی می خواهیم بدونیم getter , setter  کجاست ، باید داخل فانکشن ها رو هم چک کنیم

این درحالی هست که برای تایپ ولو ها این درد سر رو نداریم

###  تا جای ممکن سرویس ها استیت لس باشن

اگر هم استیت فول بودن دقیقا مثل لایه ی ریپازیتوری باهاش برخورد شه ، getter - setter - interface  داشته باشه در غیر این صورت تریس کد به شدت کثیف و سخت میشه
...
گاهی سرویسی نوشتیم و می خواییم برای راحتی کاربران ، به اون یه helper  اضافه کنیم تا کاربرا خیلی راحت بتونن با api  های سرویس ما کار کنن ، به جای این که اونا رو پیاده سازی کنن ، همچنین در صورتی که ما تغییرات بدیم ، تنها کافیه اونا کلاینت رو آپدیت کنن

ارض کنیم یا پروتوباف داریم ، یا استراکت برای آنمارشال کردن ، سوال : کجا مناسبه اینا رو بزاریم؟

فرض کنیم DTO  ها باید هم توی SDK ها باید تغییر پیدا کنن و هم توی سرور  ، بهتره یه جا باشن .کجا باشن؟

بهتره سمت SDK  باشن زیرا گاهی ما نمی خواییم کلاینت ها وقتی SDK ها رو ایمپورت می کنن ، SDK ها هم غیر مستقیم پروژّ اصلی یا سرور رو ایمپورت کنن ، در این صورت امنیت میاد پایین و به همه نمی تونیم SDK  رو بدیم ، بهترین حالت اینه یه SDK  کوچولو با همون زبان بنویسیم ، تست ها رو همون تو بنویسیم و در نهایت به هر کلاینتی بدیم راحت می تونه با ما ارتباط بگیره ، تازه نیاز به SWager  هم ندارهپروژه ی سرور رو ایمپورت 


**مثلث**

روی هر دو --- تنها روی SDK ---  تنها رو server

---

جواب ایده آل اینه که راه چهارمی باشه که پروتوباف داره انجام می ده ، باید از یه چیز سومی جنریت بشه تا وابستگی مستقیم نباشه ، همچنین بتونن با نسخه های قدیمی هم کار کنن

حالا که فهمیدیم مانند بیزینیس هایی که سرویس api  میدن ، باید در کنار مستنداتشون یه SDK  بدن ، آیا ما هم می تونیم راحت و سریع بدیم؟

می شه همون طور که grpc  بر اساس protobuff  هست و ربطی به زبون نداره ، می تونیم توی rest  هم از چیزی شبیه این استفاده کنیم

#### OpenAPI 3.0

می تونیم با این استاندارد توی yaml بریزیم و خروجی هر زبونی بگیریم 

پس چه با مزه میشه لایه ی adapter رو اول با yaml بنویسیم و جنریت اون رو استفاده کنیم

به شدت به درد بیزینس هایی که api  می دن ، همچنین مجموعه هایی که میکروسرویس هستن اما grpc  نیستن می خوره
