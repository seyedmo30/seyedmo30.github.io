...
گاهی می خواییم از ماشین dev  یه پورت باز کنیم و توی ماشین خودمون دسترسی داشته باشیم

مثلا یکی داکرایز چند تا سرویس اورده بالا ولی پورتاشو به بیرون  اگزپوس نکرده

این ابزار یه پورت  می ده مثلا ۴۰۴۰ و درخواست های بیرون ماشین رو به اون پورت پرایویت می ده
...
# خفن ترین پروکسی از ماشین خودمون به   dev یا docker دولوپمنت

مثلا من دهنم سرویس  با کافکای dev  کار کنم و یا یا grpc  دو کار کنم

راحت ترین روش

مثلا وقتی می خواستم grpc  بزنم یا خیلی اذییت می کرد و اولین درخواست زده می شد ، بعدیا نمیرفت ، یا کافکا کانکشنش متا دیتا و اینا داره

 `ssh -N -L 127.0.0.2:9092:localhost:9092 root@DEV_IP`

حتی میشه توی /etc/hosts نتورک داخلی اون داکر رو تعریف کنیم و به اون بزنیم

`ssh -N -L 127.0.0.3:50051:localhost:50051 root@65.108.210.47`



`/etc/hosts`

اینو اضافه می کنیم و تمام

`127.0.0.2 exchange_kafka`
