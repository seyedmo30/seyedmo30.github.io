...
2. کلاینت برای بررسی وضعیت باید **polling** کند یا از روش‌های push مانند **server-send-event(sse)** یا webhook استفاده شود.
...
| چندین مصرف‌کننده مستقل | خیر (یک مصرف‌کننده در هر پیام) | بله (چند سابسکرایبر می‌توانند همان پیام را بخوانند) |
...
* ا **At-least-once delivery** نیاز به راه‌هایی برای جلوگیری از پردازش دوباره دارد:
  ...
  * `request_id` منحصربه‌فرد که در Redis یا دیتاستوری دیگری ثبت و بررسی می‌شود.
...
* تقریباً مشابه tracing در الگوی sync است: سِت کردن trace id، انتشار آن در هدر پیام و دنبال کردن مسیر پیام از تولید تا مصرف.
...
* تنظیم replication مناسب (حداقل ۳ broker توصیه‌شده).
* فعال‌سازی `enable.idempotence=true` در Producer.
* برای Consumer: `enable.auto.commit=false` و مدیریت دستی commit.
* کش موقت تولیدکننده‌ها یا MirrorMaker برای بازنگهداری پیام‌ها (مثلاً در Redis) در مواقع بحرانی.
...
* ا **Exactly-once:** با تنظیمات ویژه و پیاده‌سازی درست (مثلاً transactional producers/consumers) می‌توان به نزدیک Exactly-Once رسید.
